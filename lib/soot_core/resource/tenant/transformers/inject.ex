defmodule SootCore.Resource.Tenant.Transformers.Inject do
  @moduledoc false
  use Spark.Dsl.Transformer

  alias Ash.Resource.Builder
  alias SootCore.Resource.Tenant.Preparations
  alias Spark.Dsl.Transformer

  @statuses [:active, :suspended, :archived]

  @impl true
  def before?(Ash.Resource.Transformers.CachePrimaryKey), do: true
  def before?(_), do: false

  @impl true
  def transform(dsl_state) do
    domain = Transformer.get_persisted(dsl_state, :domain) || domain_from_dsl(dsl_state)

    with {:ok, dsl_state} <- add_attributes(dsl_state),
         {:ok, dsl_state} <- add_identities(dsl_state, domain),
         {:ok, dsl_state} <- add_actions(dsl_state) do
      add_code_interface(dsl_state)
    end
  end

  defp domain_from_dsl(dsl_state) do
    Transformer.get_option(dsl_state, [:resource], :domain)
  end

  defp add_attributes(dsl_state) do
    with {:ok, dsl_state} <- ensure_uuid_primary_key(dsl_state),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :slug, :string,
             description:
               "URL-safe tenant identifier; appears in topics, SANs, ClickHouse row policies.",
             allow_nil?: false,
             public?: true,
             constraints: [match: ~r/^[a-z][a-z0-9-]{1,62}$/]
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :name, :string,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :status, :atom,
             constraints: [one_of: @statuses],
             default: :active,
             allow_nil?: false,
             public?: true
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :issuing_ca_id, :uuid,
             public?: true,
             description: "AshPki.CertificateAuthority used to issue this tenant's device certs."
           ),
         {:ok, dsl_state} <-
           Builder.add_new_attribute(dsl_state, :metadata, :map,
             public?: true,
             default: %{}
           ),
         {:ok, dsl_state} <- Builder.add_new_create_timestamp(dsl_state, :inserted_at) do
      Builder.add_new_update_timestamp(dsl_state, :updated_at)
    end
  end

  defp ensure_uuid_primary_key(dsl_state) do
    if Ash.Resource.Info.attribute(dsl_state, :id) do
      {:ok, dsl_state}
    else
      Builder.add_new_attribute(dsl_state, :id, :uuid,
        primary_key?: true,
        allow_nil?: false,
        public?: true,
        default: &Ash.UUID.generate/0,
        match_other_defaults?: true
      )
    end
  end

  defp add_identities(dsl_state, domain) do
    Builder.add_new_identity(dsl_state, :unique_slug, [:slug], pre_check_with: domain)
  end

  defp add_actions(dsl_state) do
    with {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :read, :read, primary?: true),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :destroy, :destroy, primary?: true),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :create, :create,
             primary?: true,
             accept: [:slug, :name, :issuing_ca_id, :metadata]
           ),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :read, :get_by_slug,
             arguments: [
               Builder.build_action_argument(:slug, :string, allow_nil?: false)
             ],
             get?: true,
             preparations: [Builder.build_preparation(Preparations.GetBySlug)]
           ),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :update, :suspend,
             accept: [],
             require_atomic?: false,
             changes: [
               Builder.build_action_change(
                 {Ash.Resource.Change.SetAttribute, attribute: :status, value: :suspended}
               )
             ]
           ),
         {:ok, dsl_state} <-
           Builder.add_new_action(dsl_state, :update, :reactivate,
             accept: [],
             require_atomic?: false,
             changes: [
               Builder.build_action_change(
                 {Ash.Resource.Change.SetAttribute, attribute: :status, value: :active}
               )
             ]
           ) do
      Builder.add_new_action(dsl_state, :update, :archive,
        accept: [],
        require_atomic?: false,
        changes: [
          Builder.build_action_change(
            {Ash.Resource.Change.SetAttribute, attribute: :status, value: :archived}
          )
        ]
      )
    end
  end

  defp add_code_interface(dsl_state) do
    with {:ok, dsl_state} <- Builder.add_new_interface(dsl_state, :create, args: [:slug, :name]),
         {:ok, dsl_state} <- Builder.add_new_interface(dsl_state, :get_by_slug, args: [:slug]),
         {:ok, dsl_state} <- Builder.add_new_interface(dsl_state, :suspend),
         {:ok, dsl_state} <- Builder.add_new_interface(dsl_state, :reactivate) do
      Builder.add_new_interface(dsl_state, :archive)
    end
  end
end
